...
function getWeeklyTheme(age) {
  const stageThemes = {
    ...
    2: [
      { theme: "小小探险家", description: "爬行让世界变得立体！宝宝开始主动探索空间，因果认知飞速发展", fatherTip: "爸爸是天然的'冒险鼓励者'。当妈妈可能说'小心'，爸爸更倾向说'去试试'——这种互补正是完整依恋系统的设计。" },
      { theme: "因果世界", description: "按→响、扔→掉、推→倒……每一次重复都是科学家精神的萌芽", fatherTip: "她反复扔玩具不是捣乱！是在验证'每次扔都会掉'这个物理定律。你配合捡起来还给她=世界上最有耐心的实验助手。" },
    ],
    3: [
...
function getWeekendTheme(age) {
  const themes = {
    ...
    2: { theme: "探索大冒险", description: "工作日练基本功，周末进入实战——爬行、攀爬、户外探索全面展开", fatherValue: "爸爸天然的游戏风格更具冒险性和不可预测性。周末的大运动游戏——爬行障碍赛、身体翻转、追逐游戏——正是父亲角色不可替代的贡献。这些经历帮助宝宝建立身体自信和风险评估能力。" },
...
function getWeekHighlight(age) {
  const highlights = {
    ...
    2: "主动探索期的宝宝每天都在刷新你的认知：昨天还只能在原地打转，今天可能就开始向前爬了。每一次'够到了'的成功体验，都在建立她的自我效能感——'我能做到'的信念，比任何技能都重要。",
...
function getNextWeekPreview(age) {
  const previews = {
    ...
    2: "下周的重点是巩固爬行技能并开始引入更复杂的因果玩具。同时，我们会加入更多坐姿平衡训练——为未来的站立做准备。",
